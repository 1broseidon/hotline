...
fn descriptors() -> Vec<Tool> {
    vec![
        ...
        Tool::new(
            LIST_TEAMMATES,
            "The other teammates in this Toad room: each one's id and public name. Roster metadata only — it does not include session state, workspaces, tools, or anyone's conversation.",
            schema(json!({ "type": "object", "properties": {}, "additionalProperties": false })),
        ),
        Tool::new(
            MESSAGE_TEAMMATE,
            "Ask another teammate in this room something, and get their answer back. The call waits for their reply, so ask for one specific thing and say everything they need: they cannot see your conversation with the user, and they answer in one turn without a follow-up. Workspace callers need the operator's first-contact approval before the recipient uses its workspace and enabled tools; Whole machine Toad Agent callers can initiate directly. They are started if they are not running. The two of you have a standing private thread, and they can see what was said in it before.",
            schema(json!({
                "type": "object",
                "properties": {
                    "to": {
