...
use hotline_core::paths::data_root;
...
fn install_menu(app: &tauri::App) -> tauri::Result<()> {
    let settings = MenuItemBuilder::with_id("settings", "Settings…")
        ...
        .build(app)?;
    let about = MenuItemBuilder::with_id("about", "About Hotline").build(app)?;
    let app_menu = SubmenuBuilder::new(app, "Hotline")
        .item(&about)
        .separator()
        .item(&settings)
        .separator()
    ...
    let github = MenuItemBuilder::with_id("github", "Hotline on GitHub").build(app)?;
    ...
    app.on_menu_event(move |_app, event| {
        ...
        {
            let _ = handle.emit("hotline://menu", id);
        }
    });
    Ok(())
}
...
fn install_tray(app: &tauri::App) -> tauri::Result<()> {
    let open = MenuItemBuilder::with_id("open", "Open Hotline").build(app)?;
    let quit = MenuItemBuilder::with_id("quit", "Quit Hotline").build(app)?;
    let menu = MenuBuilder::new(app)
        .item(&open)
        .separator()
        .item(&quit)
