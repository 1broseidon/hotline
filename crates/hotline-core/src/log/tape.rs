...
//!
//! This layout is the previous edition's, spelled the same way on purpose: a tape
//! this writes is one that Hotline reads unchanged, so importing a data directory
...
///
/// The previous edition took this from the teammate's record, because a hop
...
/// written by that Hotline, past a hop — are the whole truth of where the tape is
...
mod tests {
    ...
    fn scratch(name: &str) -> (PathBuf, Log) {
        let root =
            std::env::temp_dir().join(format!("hotline-core-tape-{name}-{}", std::process::id()));
    ...
    #[test]
    fn a_tape_this_writes_is_byte_for_byte_the_one_the_previous_hotline_writes() {
