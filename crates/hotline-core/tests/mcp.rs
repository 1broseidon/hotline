//! Hotline as an MCP client, driven over the wire the window uses.
//!
...
use hotline_core::mcp::{self, McpServer, McpTransport};
...
/// Hotline's own tools are the teammate's whether or not anything else is:
/// built in this process for Hotline Agent, and verified because Hotline built them.
#[tokio::test(flavor = "multi_thread")]
async fn hotline_agent_gets_hotlines_own_tools() {
    let (_root, port) = open("hotline-tools").await;
    let mut client = Client::connect(port).await;
    keyed(&mut client).await;
    ...
    let tools = client
        ...
        .await;
    let rows = tools["result"]["rows"].as_array().unwrap();
    for name in hotline_core::mcp::server::TOOL_NAMES {
        let row = rows
            .iter()
...
/// Killing only the process Hotline spawned reparents that child to pid 1, where
/// it keeps whatever it was holding for as long as Hotline runs. Here a
