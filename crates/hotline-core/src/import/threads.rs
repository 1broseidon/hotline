//! Peer threads, copied from the previous edition's `threads/` directory.
//!
...
//! Hotline's, so the copy is the files themselves, not a rewrite.
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!(
            "hotline-core-import-threads-{name}-{}",
            std::process::id()
        ));
