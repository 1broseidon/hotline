...
Prefer a plugin? `claude plugin marketplace add 1broseidon/hotline`, then `claude plugin install mission-control@hotline`, then run `/mission-control:init` in the target folder to scaffold the same files.
