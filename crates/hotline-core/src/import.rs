//! Import an earlier edition's data directory into this room. The layout read
//! here is the one the Electrobun edition wrote — `store.sqlite`, tapes and a
//! file vault — under the name Toad, which is what that edition was called.
//!
...
/// Old Hotline backend id → this registry's id, for every DEFAULT harness both
/// trees name as the same agent. Evidence is the previous edition's
...
///
/// `pi` → `hotline`. Hotline Agent. The previous edition's `PI_BACKEND_ID` /
/// `DEFAULT_BACKEND_ID`; this tree's `driver::HOTLINE_BACKEND_ID`. Not in the
...
/// This registry's id for an old Hotline backend, when both trees taught the
/// same harness.
...
/// Copies the previous edition's roster, tapes, settings and keys into `log` and
...
fn import_teammates(
    ...
    for value in personas::list_foreign_personas_from(store_root, workspace_root) {
        ...
        report.skipped.push(Skipped {
            ...
            reason: "lives on another desk; this Hotline has no fleet".into(),
        });
    }
    ...
    for (key, value) in settings {
        if !IMPORTED_SETTINGS.contains(&key.as_str()) {
            report.skipped.push(Skipped {
                ...
                reason: "not a setting this Hotline has".into(),
            });
            continue;
        }
...
fn setting_value_to_import(key: &str, value: Value, report: &mut Report) -> Option<Value> {
        ...
        return match value.as_str().and_then(counterpart) {
            ...
            None => {
                report.skipped.push(Skipped {
                    ...
                    reason: "not a backend this Hotline has".into(),
                });
                None
            }
        };
    ...
    let Some(entries) = value.as_array() else {
        report.skipped.push(Skipped {
            ...
            reason: "not a server list this Hotline can read".into(),
        });
        return None;
    };
    ...
    for entry in entries {
        ...
        if normalised.is_empty() {
            report.notes.push(Skipped {
                ...
                reason: "not a server this Hotline can read".into(),
            });
        } else {
...
fn import_keys(
    ...
    if report.keys == 0 {
        const GUIDANCE: &str = "no Anthropic, OpenAI or OpenRouter key came over; add one in Keys before a Hotline Agent teammate can answer";
...
fn snapshot_dir(from: &Path) -> PathBuf {
    ...
    std::env::temp_dir().join(format!("hotline-import-store-{:016x}", hasher.finish()))
}
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("hotline-core-import-{name}-{}", std::process::id()));
    ...
    fn an_old_data_directory_comes_over_once_and_the_source_is_untouched() {
        ...
        assert!(
            reasons.iter().any(|(item, reason)| *item == "setting theme"
                && *reason == "not a setting this Hotline has"),
            "{reasons:?}"
        );
        assert!(
            reasons
                ...
                .any(|(item, reason)| *item == "teammate Theirs"
                    && *reason == "lives on another desk; this Hotline has no fleet"),
            "{reasons:?}"
        );
        assert!(
            reasons
        ...
        assert_eq!(settings["defaultBackendId"], "hotline");
        ...
        assert!(
            report
                ...
                .any(|note| note.item == "mcp server no-name"
                    && note.reason == "not a server this Hotline can read"),
            "{report:?}"
        );
    }
            ...
            report.skipped.iter().any(|skipped| {
                skipped.item == "key claude-login (anthropic)"
                    ...
                    && skipped.reason.contains(
                        "no Anthropic, OpenAI or OpenRouter key came over; add one in Keys before a Hotline Agent teammate can answer",
                    )
            }),
            "{report:?}"
        );
