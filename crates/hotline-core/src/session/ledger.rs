...
const NO_REASON: &str = "no reason was recorded, which is itself a bug in Hotline";
...
/// What Hotline knows about this teammate's tools. `None` when it has never
/// started under a Hotline that keeps a ledger — which is itself worth saying
...
impl ToolLedger {
    ...
    /// Hotline watched the agent take this tool.
    pub fn verified(
        &mut self,
        source: ToolSourceKind,
        origin: impl Into<String>,
    ...
    /// Hotline handed this tool over and cannot see whether the agent took it.
    pub fn declared(
        &mut self,
        source: ToolSourceKind,
        origin: impl Into<String>,
...
mod tests {
    ...
    fn every_row_carries_a_reason_in_every_state() {
        let mut ledger = ToolLedger::new("p1", AgentKind::Hotline, "hotline");
        ledger
            .verified(
                ToolSourceKind::Builtin,
                "hotline",
                "read",
                "a built-in of the Hotline Agent runtime",
            )
            .declared(
                ToolSourceKind::Mcp,
                "Echo",
            ...
            .absent(
                ...
                "Hotline",
                "hop_desk",
                "this Hotline does not own the bridge socket",
            )
            .publish();
    ...
    fn an_empty_reason_becomes_a_loud_one_rather_than_an_empty_cell() {
        let mut ledger = ToolLedger::new("p2", AgentKind::Hotline, "hotline");
        ledger
            .absent(ToolSourceKind::Mcp, "Echo", "shout", "   ")
            .publish();
        let reason = &teammate_tools("p2").unwrap().rows[0].reason;
        assert!(reason.contains("bug in Hotline"), "{reason}");
    }

    #[test]
    fn the_same_tool_from_two_suppliers_is_two_rows() {
        let mut ledger = ToolLedger::new("p3", AgentKind::Hotline, "hotline");
        ledger
    ...
    fn a_supplier_that_goes_away_turns_its_rows_absent_with_one_cause() {
        let mut ledger = ToolLedger::new("p5", AgentKind::Hotline, "hotline");
        ledger
    ...
    fn mentioning_names_every_teammate_whose_ledger_holds_the_supplier() {
        let mut a = ToolLedger::new("mention-a", AgentKind::Hotline, "hotline");
        ...
        b.declared(ToolSourceKind::Mcp, "EchoMention", "t", "handed over")
            .publish();
        let mut c = ToolLedger::new("mention-c", AgentKind::Hotline, "hotline");
        c.verified(ToolSourceKind::Builtin, "hotline", "read", "built-in")
            .publish();
