...
Tokens, allowlists, pending pairings, and transcripts live in the state directory (`${XDG_CONFIG_HOME:-~/.config}/hotline` by default; older installs are migrated there automatically on first run), outside any repository. The unnamed box retains that legacy root; named boxes isolate Mission Control, schedules, loops, notify state, jobs, and supervisor state under `bots/<name>/`. Directories are created `0700`; `access.json`, the transcript, and the `.env` token file are written `0600`. `access.json` writes are atomic (temp file plus rename) and lock-guarded. Runtime owner locks are held for the process lifetime over the box root, each provider state directory, and Mission Control, so another live box refuses rather than reading or writing those resources. `bot.pid` is diagnostic only; its sibling flock is the provider-consumer liveness signal. Outbound tool calls are gated by the same access rules, refuse to attach hotline's own state files, and sanitize uploader-controlled filenames.
