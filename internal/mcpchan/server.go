...
How their messages reach you: inbound text arrives in the <channel> block. image_path means Read that file (a photo they attached); attachment_file_id means call download_attachment, then Read the path it returns. Pass chat_id back on every reply. Use reply_to (a message_id) only when answering an older message, not their latest. Telegram has no history or search — if you need earlier context, ask them to paste it.
