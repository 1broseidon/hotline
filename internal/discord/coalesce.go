...
// Inbound coalescing — the same burst-buffering model as the telegram adapter
// (people text in fragments; deliver a burst as ONE turn). Scoped per provider
// as the architecture intends: each transport buffers its own inbound stream.
const (
...
// chatBuffer accumulates a burst for a single channel.
