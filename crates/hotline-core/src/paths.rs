//! Where Hotline keeps its state, spelled exactly the way the Bun main spells it.
//!
...
/// The data directory: `HOTLINE_DATA_DIR`, or the platform's application
/// support directory.
pub fn data_root() -> PathBuf {
    if let Ok(dir) = env::var("HOTLINE_DATA_DIR")
        && !dir.trim().is_empty()
    {
...
/// so only a tape needs them. It replaces the previous edition's `store.sqlite`
...
/// the agents Hotline was taught by hand.
...
/// actually live, and a computer that cannot start because Hotline was opened
...
/// that is how the sidecar is written, here and in the previous edition — so
