...
#[cfg(target_os = "macos")]
fn main() {
    use hotline_core::driver::acp::registry::cached_backends;
    use std::os::unix::fs::PermissionsExt;
    use std::process::Command;

    if std::env::var_os("HOTLINE_PATH_TEST_CHILD").is_some() {
        let root = std::env::var_os("HOTLINE_DATA_DIR").unwrap();
    ...
    let status = Command::new(std::env::current_exe().unwrap())
        .env("HOTLINE_PATH_TEST_CHILD", "1")
        .env("HOTLINE_DATA_DIR", root.path())
        .env("HOME", root.path())
        .env("SHELL", shell)
