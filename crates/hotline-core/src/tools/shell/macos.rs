...
pub(super) fn command(command: &str, workspace: &Path) -> Result<Command, String> {
    ...
    process.args(["/bin/sh", "-c", &setup, "hotline-shell", command]);
    Ok(process)
}
...
mod tests {
    ...
    async fn host_process_environment_and_signals_are_not_available() {
        ...
        let mut host = std::process::Command::new("/bin/sleep")
            .arg("30")
            .env("HOTLINE_HOST_CANARY", "host-process-secret")
            .spawn()
            .unwrap();
        let pid = host.id();
    ...
    async fn private_scratch_and_hostile_setup_symlinks() {
        ...
        symlink(&outside, work.join(".hotline-home")).unwrap();
        assert!(!output(&work, "echo should-not-run").await.status.success());
        assert!(!outside.exists());
        std::fs::remove_file(work.join(".hotline-home")).unwrap();
        std::fs::create_dir(work.join(".hotline-home")).unwrap();
        symlink(&outside, work.join(".hotline-home/.tmp")).unwrap();
        assert!(!output(&work, "echo should-not-run").await.status.success());
        assert!(!outside.exists());
        std::fs::remove_file(work.join(".hotline-home/.tmp")).unwrap();
        let result = output(
            &work,
    ...
    async fn package_downloads_use_private_caches() {
        ...
        for (tool, script, cache) in [
            ...
            assert!(root.path().join(".hotline-home").join(cache).exists());
        }
    }
