...
//!
//! Written to be byte-for-byte what the previous edition's `store/threads.ts`
...
/// Hotline parses it with a cast, which keeps every field the file had, so a
...
/// half-written one. The pid in the name is what keeps two copies of
/// Hotline from picking the same temporary.
...
/// which is what the previous edition writes and what decides which way round a
...
mod tests {
    ...
    fn scratch(name: &str) -> (PathBuf, Log) {
        let root =
            std::env::temp_dir().join(format!("hotline-core-thread-{name}-{}", std::process::id()));
    ...
    /// The bytes below came out of the previous edition's own writer. Produced by
    /// running, against a throwaway `HOTLINE_DATA_DIR`, a script that calls
    ...
    #[test]
    fn a_thread_this_writes_is_byte_for_byte_the_one_the_previous_hotline_writes() {
