...
//!
//! The instructions and the note's shape are the previous edition's, unchanged,
...
pub(super) const INSTRUCTIONS: &str = r#"You write the handoff note that closes one chapter of an ongoing conversation between a person and their teammate, an AI agent working in a desktop app called Hotline. The next chapter starts with a fresh context and reads only your note, so it must carry the few things that matter and nothing else.
...
/// makes it the open one, and no `sessionId`, because Hotline Agent's context
...
/// Hotline's words to the session that just reopened: the user lines said in
/// the chapter that is being left, never written to the tape. The wording is
/// the previous edition's, because it was written for agents.
pub(super) fn resume_nudge(said: &[String]) -> String {
    let body = if said.is_empty() {
        ...
        format!(
            ...
            crate::fence::fenced(
                "hotline_user_messages",
                &Value::from(said.to_vec()).to_string()
            )
        )
    };
    format!(
        "Hotline has reopened this chapter's context at the user's request, so you remember the work above. {body}Do not mention the reopening or this note."
    )
}
...
/// from the tape, because Hotline explaining the room to the agent is machinery
...
pub(super) fn wake_block(events: &[Value], now: i64) -> Option<String> {
    ...
    // note. The seed (Hotline Agent) or the checkpoint (ACP) is the conversation
...
mod tests {
    ...
    fn a_close_supersedes_the_marker_by_id_and_keeps_when_it_opened() {
        let open = json!({"kind": "chapter", "id": "c1", "ts": 100, "backendId": "hotline"});
        let note = Note {
    ...
    fn the_wake_block_carries_the_note_the_gap_and_the_tone() {
        let events = [
            json!({
                "kind": "chapter", "id": "c1", "ts": NOW - 100_000, "backendId": "hotline",
    ...
    fn a_resume_nudge_quotes_the_interim_user_lines_and_never_asks_the_agent_to_mention_it() {
        ...
        assert!(
            with.contains("<hotline_user_messages>\n[\"and now?\"]\n</hotline_user_messages>"),
            "{with}"
        );
    ...
    fn a_resumed_chapter_wakes_on_the_note_it_carries() {
        let events = [
            ...
            user("u1", NOW - 7_300_000, "did it hold?"),
            json!({
                "kind": "chapter", "id": "c2", "ts": NOW - 1_000, "backendId": "hotline",
