...
//!   previous edition wrote — [`crate::contract::TranscriptEvent`] pins them, and
...
use crate::driver::{
    CapabilityEpoch, CapabilityLease, Driver, DriverInfo, HOTLINE_BACKEND_ID, MessageKind, Update,
    clip,
};
...
/// in the previous edition a prompt could be refused between leaving the mark and
...
/// because a chapter that went stale while Hotline was closed should be closed
...
pub trait ProviderKeys: Send + Sync {
    ...
    /// The model a Hotline Agent teammate starts on when it has no choice of
...
/// The agents this desk can really run: Hotline Agent on whatever keys the desk
...
impl Room {
    ...
    async fn start_now(
        ...
        // Hotline draws the permission cards but does not decide whether the
    ...
    /// again: Hotline Agent is seeded from the previous chapter's tape slice,
    ...
    pub async fn resume_chapter(
        ...
        // session, which is what an ACP child will try to resume. Hotline Agent
        // has no checkpoint; its restore is the tape slice `said` will seed.
        {
            ...
            } else if persona.backend_id != HOTLINE_BACKEND_ID
                && let Err(error) =
    ...
    async fn close_chapter(
        ...
        // Hotline was closed — so "ended nine hours ago" on wake means what it
        // says.
    ...
    fn needs_you(&self, persona_id: &str) -> String {
        match self.persona(persona_id) {
            ...
            Err(_) => "Hotline needs you".to_string(),
        }
    }
...
/// opens, which is where a chapter that went stale while Hotline was closed is
...
/// earlier stretch plus anything said since, because Hotline Agent has no
...
/// previous edition's, and there is nowhere for a second copy of them to drift
...
pub(crate) fn preamble(
    ...
    // Every teammate has Hotline's own tools — over its conversation and over
