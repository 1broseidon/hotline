...
//! provider key in `HOTLINE_HARNESS_ANTHROPIC_KEY` and is skipped without one,
...
async fn legacy_mcp_secrets_are_hidden_and_repair_removes_the_plaintext_history() {
    ...
    let log = hotline_core::log::Log::open(&root);
    log.append(&hotline_core::log::StreamId::Room, &json!({
...
fn scratch(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("hotline-harness-{name}-{}", std::process::id()));
...
/// teammate's own workspace skills are listed without the ones Hotline copied
...
async fn the_skills_catalog_lists_builtins_the_gateway_and_a_teammates_own() {
    ...
    .unwrap();
    std::fs::write(
        workspace.join(".agents/skills/copied/.managed-by-hotline"),
        "",
    )
    .unwrap();
    ...
    assert_eq!(
        summary,
        [
            ("builtin".to_string(), "hotline-room".to_string(), None),
            (
                "gateway".to_string(),
                "Broken".to_string(),
                Some(
    ...
    assert_eq!(entries[0]["path"], ".agents/skills/hotline-room/SKILL.md");
    ...
    assert!(folder.join("hotline-room/SKILL.md").exists());
    assert!(folder.join("hotline-room/.managed-by-hotline").exists());
    ...
    assert!(
        ...
        "a stale copy Hotline made is removed"
    );
    assert!(
    ...
    assert!(
        ...
        "a revoked grant leaves nothing of Hotline's"
    );
    assert!(folder.join("hotline-room/SKILL.md").exists());
...
async fn a_teammate_is_made_watched_keyed_chaptered_and_removed_over_the_wire() {
    ...
    assert_eq!(created["ok"], true, "{created}");
    let persona_id = created["result"]["id"].as_str().unwrap().to_string();
    assert_eq!(created["result"]["backendId"], "hotline");
    ...
    let marker = client
        ...
        .await;
    assert_eq!(marker["event"]["backendId"], "hotline");
...
async fn the_welcome_state_derives_from_a_fresh_room_to_a_first_teammate() {
    ...
    assert!(
        harnesses
            .iter()
            .all(|one| one["id"] != "hotline" && one.get("unavailable").is_none()),
    ...
    if let Some(harness) = harnesses.first() {
        ...
        let back = client
            .call(
                "settings.update",
                json!({ "patch": { "defaultBackendId": "hotline" } }),
            )
            .await;
...
async fn a_peer_thread_is_listed_streamed_and_marked_read_over_the_wire() {
    ...
    let teammate = |id: &str, name: &str| {
        json!({
            "kind": "persona", "id": id, "name": name, "goal": "", "backendId": "hotline",
...
async fn a_turn_with_a_real_key_reads_a_file_and_answers_from_it() {
    let Ok(key) = std::env::var("HOTLINE_HARNESS_ANTHROPIC_KEY") else {
        eprintln!("skipped: set HOTLINE_HARNESS_ANTHROPIC_KEY to run a real turn");
        return;
    };
    ...
    let kinds: Vec<&str> = log
        .load(&hotline_core::log::StreamId::Tape(persona_id.clone()))
        .iter()
...
/// installed and logged in is a fact about the machine and not about Hotline.
#[tokio::test(flavor = "multi_thread")]
async fn a_turn_on_a_real_acp_harness_reaches_the_tape() {
    let Ok(backend_id) = std::env::var("HOTLINE_HARNESS_ACP") else {
        eprintln!(
            "skipped: set HOTLINE_HARNESS_ACP to a backend id (e.g. cursor) to drive a harness"
        );
        return;
    };
    ...
    assert!(
        identity.starts_with("<!-- managed by Hotline -->"),
        "{identity}"
    );
    ...
    let log = hotline_core::log::Log::open(&root);
    let checkpoints: Vec<String> = hotline_core::room::roster(&log)
        .into_iter()
...
async fn workspace_reach_keeps_another_projects_env_out_of_the_tools() {
    use hotline_core::contract::Persona;
    use hotline_core::tools::{ReadFile, RunCommand, Workspace};
    ...
    hotline_core::tools::shell_available(hotline_core::contract::Reach::Workspace).unwrap();
...
async fn ollama_local_discovers_custom_models_and_runs_through_rig() {
    ...
    assert_eq!(
        hotline_core::wire::RoomHandle::models(&reopened)[0].id,
...
async fn custom_connections_keep_models_keys_and_edits_separate() {
    ...
    assert!(!auth.exists());
    let reopened = common::open_desk(&root).unwrap();
    assert_eq!(hotline_core::wire::RoomHandle::models(&reopened).len(), 2);
    drop(reopened);
...
/// next request contains the file contents from Hotline's actual workspace tool.
...
async fn discovery_and_manual_ids_preserve_filters_selections_and_offline_data_over_the_wire() {
    ...
    use hotline_core::wire::RoomHandle;
    use std::sync::Mutex;
    ...
    *ids.lock().unwrap() = vec!["new-coder".into()];
    ...
    let persisted = hotline_core::room::settings(&reopened.log);
...
async fn native_connection_metadata_and_account_restrictions_survive_reopening_over_the_wire() {
    use hotline_core::{log::Log, wire::RoomHandle};
    ...
    vault
        ...
        .unwrap();
    let known = hotline_core::models::catalog().providers["anthropic"]
        .models
        .keys()
        .next()
        .unwrap()
    ...
    let discovery = serde_json::to_vec(&json!([
        ...
        {"id":"released-after-hotline","name":"New provider model","output_limit":4096}
    ]))
    .unwrap();
...
async fn the_desktop_restart_lease_refuses_new_wire_work_and_keeps_saved_data() {
    ...
    let saved = desk.log.load(&hotline_core::log::StreamId::Room);
    ...
    assert_eq!(desk.log.load(&hotline_core::log::StreamId::Room), saved);
    drop(held);
    assert!(desk.prepare_restart().is_ok());
    door_task.abort();
    let reopened = common::open_desk(&root).unwrap();
    assert_eq!(reopened.log.load(&hotline_core::log::StreamId::Room), saved);
}
