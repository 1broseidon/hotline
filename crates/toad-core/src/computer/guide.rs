...
//!
//! toad.computer bundles a skill for the release it is and serves it from
//! `state {"action":"guide"}` with its version and a checksum. Toad never
...
//! `toad-computer` skill. A computer that cannot answer — an older image
...
fn parse(text: &str) -> Result<Guide, String> {
    let manifest: Value = serde_json::from_str(text)
        .map_err(|_| "The computer's guide was not the manifest Toad expects.".to_string())?;
    let field = |name: &str| {
        manifest
            .get(name)
...
pub(crate) mod fake {
    ...
    /// filled the way toad.computer's own build fills it.
    pub(crate) fn skill_of(version: &str) -> String {
        format!(
            "---\nname: toad-computer\ndescription: Operate a Toad Computer through its MCP tools.\n---\n\nThis guide ships with Toad Computer {version}.\n"
        )
    }
