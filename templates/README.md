...
To use one today, copy the template folder's contents into the project you want to text with (or make the folder itself the project), then run `hotline init` there and `hotline start` as usual. The agent picks up the `CLAUDE.md` and `HOTLINE.md` on launch and starts working the system from the first message.
