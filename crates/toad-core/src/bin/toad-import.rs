...
/// ancestor that does, so `toad-import X X/new` is refused before the
/// log creates `X/new` inside the source.
fn overlap_reason(from: &Path, to: &Path) -> Option<String> {
    if from == to {
        return Some(format!(
            "{} is the source directory or lives inside it; the importer would write into the Toad it is reading",
            to.display()
        ));
    }
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("toad-import-bin-{name}-{}", std::process::id()));
