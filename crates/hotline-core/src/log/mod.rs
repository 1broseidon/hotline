...
mod tests {
    ...
    fn scratch(name: &str) -> Log {
        let root =
            std::env::temp_dir().join(format!("hotline-core-log-{name}-{}", std::process::id()));
    ...
    fn the_old_backend_id_is_renamed_on_disk_once_and_prose_is_left_alone() {
        ...
        assert!(
            fs::read_to_string(root.join("transcripts").join("bob.jsonl"))
                .unwrap()
                .contains(r#""backendId":"hotline""#)
        );
        assert_eq!(log.load(&StreamId::Room)[0]["backendId"], "hotline");
    ...
    fn a_clone_of_the_log_is_the_same_log() {
        ...
        let mut listener = log.subscribe(&StreamId::Room);
        let event = json!({"kind": "setting", "id": "defaultBackendId", "value": "hotline"});
