        ...
        get(move |uri: Uri| {
            ...
            async move {
                ...
                (
                    ...
                    "Return to Hotline to finish signing in. You can close this page.",
                )
            }
        }),
    );
...
mod tests {
    ...
    async fn openrouter_pkce_exchanges_once_and_keeps_the_key_private() {
        let root =
            std::env::temp_dir().join(format!("hotline-openrouter-{}", uuid::Uuid::new_v4()));
    ...
    async fn openrouter_rejects_empty_keys_and_redacts_failed_exchange_bodies() {
        for (status, body) in [
            ...
            let dir = std::env::temp_dir()
                .join(format!("hotline-openrouter-bad-{}", uuid::Uuid::new_v4()));
