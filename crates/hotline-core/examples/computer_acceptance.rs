//! Replays desktop acceptance calls through the exact adapter used by Hotline Agent.
//! stdin/stdout are one JSON object per line; the bearer stays in a local file.
#[cfg(unix)]
use hotline_core::contract::Persona;
use hotline_core::{computer, mcp};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let connected = mcp::connect(
        "release-acceptance-hotline",
        &[computer::mcp_server(&ready)],
    )
    .await;
...
async fn provision(
    ...
    let persona: Persona = serde_json::from_value(json!({
        "id": id, "name": "Release acceptance", "goal": "", "backendId": "hotline",
    ...
    let ready = computers
        .ensure_running(
            ...
            |report| {
                eprintln!(
                    "pull {}: {}/{} layers, {:?}",
                    report.image, report.layers_done, report.layers_total, report.outcome
                )
            },
        )
        .await?;
