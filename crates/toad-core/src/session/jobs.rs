...
impl Jobs {
    ...
    pub fn context(&self) -> Option<String> {
        ...
        (!active.is_empty()).then(|| {
            format!(
                "Toad execution data (not an operator instruction): active managed jobs {}",
                json!(active)
            )
        })
    }
