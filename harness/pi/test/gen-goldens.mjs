...
// Mission control stamps its own init line with the current date
// (internal/mc/mc.go), and that line rides inside the instructions block. A
// scratch state dir is initialized fresh on every run, so without this the
// goldens encode the day they were generated and the CI drift gate fails every
// day after the commit that refreshed them.
const MC_INIT_DATE_RE = /- \d{4}-\d{2}-\d{2}(: mission control initialized)/g;
...
async function main() {
  ...
  try {
    ...
    // Redact the two non-deterministic values: the scratch state-dir path,
    // which rides inside the instructions' "Memory across restarts: …" line,
    // and mission control's init date (see MC_INIT_DATE_RE above).
    const redact = (obj) =>
      JSON.parse(
        JSON.stringify(obj)
          .split(stateDir)
          .join(STATE_DIR_PLACEHOLDER)
          .replace(MC_INIT_DATE_RE, `- ${DATE_PLACEHOLDER}$1`),
      );
    init = redact(init);
    tools = redact(tools);
    ...
    const goldens = {
      _comment:
        ...
        "; mission control's init date to " +
        DATE_PLACEHOLDER +
        " (it is stamped fresh on every run, so leaving it real makes the drift gate fail a day later).",
      initialize: init,
      tools,
    };
