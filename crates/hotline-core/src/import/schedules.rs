//! Jobs from the previous edition's `schedules.json`, written onto the room
...
/// The previous edition's job, as this stream folds a schedule event.
///
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!(
            "hotline-core-import-schedules-{name}-{}",
            std::process::id()
        ));
