//! Rewrites `crates/toad-core/models.json` from models.dev.
//!
//! ```text
//! cargo run -p toad-core --bin toad-models-sync            # fetch, filter, write
//! cargo run -p toad-core --bin toad-models-sync -- --from api.json
//! ```
//!
//! What it keeps is decided in `toad_core::models`; this is only the fetch,
...
use toad_core::models::{self, Catalog};
...
async fn main() -> ExitCode {
    match run().await {
        Ok(()) => ExitCode::SUCCESS,
        Err(message) => {
            eprintln!("toad-models-sync: {message}");
            ExitCode::FAILURE
        }
    }
}
...
async fn run() -> Result<(), String> {
    ...
    let body = match (args.next().as_deref(), args.next()) {
        ...
        _ => return Err("usage: toad-models-sync [--from api.json]".to_string()),
    };
